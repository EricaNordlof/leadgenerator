...
async function api(path, options = {}) {
  ...
  try {
    ...
    if (error?.name === 'AbortError') {
      throw new Error('Servern svarade inte inom 25 sekunder. Försök igen.');
    }
...
async function loadIntegrations() {
  ...
  $('discoveryStatusText').textContent = `${sources || 'Ingen standardkälla'} är aktiv. ${discovery.officialFeeds || 0} extra officiella feed-källor är konfigurerade. Daglig körning: ${discovery.dailySchedule}.`;
}
...
async function createGmailDraft() {
  ...
  $('draftSubject').value = data.draft.subject;
  $('draftBody').value = data.draft.body;
  showToast('Utkastet skapades i Gmail. Ingenting har skickats.');
...
async function runDiscovery() {
  ...
  try {
    ...
    showToast(`${result.newLeads} nya leads lades till.`);
    state.view = 'today';
...
function renderRuns(runs) {
  $('runList').innerHTML = runs.length ? runs.map((run) => `
    <div class="run-item">
      <div><strong>${escapeHtml(run.source)}</strong><small>${formatDateTime(run.started_at)}</small></div>
      <div><span class="status-pill ${run.status === 'completed' ? 'success' : run.status === 'failed' ? 'danger' : 'neutral'}">${escapeHtml(run.status)}</span></div>
