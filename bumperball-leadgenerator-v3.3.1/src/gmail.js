...
import { normalizeDraftContent } from './draft-content.js';
export { normalizeDraftContent } from './draft-content.js';
...
export async function createLeadDraft(userId, lead, type = 'first', overrides = {}) {
  if (!lead.email) throw new Error('Leaden saknar e-postadress.');
  const { client, gmailEmail } = await authorizedClient(userId);
  const gmail = google.gmail({ version: 'v1', auth: client });
  const template = buildLeadDraft(lead, type);
  const draft = normalizeDraftContent(template, overrides);
