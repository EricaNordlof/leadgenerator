...
router.post('/leads/:id/gmail-draft', requireAuth, requireCsrf, asyncHandler(async (req, res) => {
  const draftSchema = z.object({
    type: z.enum(['first', 'followup', 'event']).default('first'),
    subject: z.string().trim().min(1).max(200).optional(),
    body: z.string().trim().min(1).max(20000).optional()
  }).superRefine((value, ctx) => {
    if (Boolean(value.subject) !== Boolean(value.body)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Både ämnesrad och meddelande måste anges när utkastet redigeras.'
      });
    }
  });
  const { type, subject, body } = draftSchema.parse(req.body || {});
  const result = await query('SELECT * FROM leads WHERE id = $1 AND archived_at IS NULL', [req.params.id]);
  if (!result.rowCount) return res.status(404).json({ error: 'Leaden hittades inte.' });
  const draft = await createLeadDraft(req.session.user.id, result.rows[0], type, { subject, body });
  await query(
