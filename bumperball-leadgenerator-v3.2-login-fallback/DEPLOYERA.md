...
## Inloggningen reagerar inte

Version 3.1 visar alltid status eller ett konkret fel. Efter uppladdning:

1. Kontrollera att `ADMIN_PASSWORD` finns på webbtjänsten.
2. Gör **Manual Deploy → Deploy latest commit**.
3. Ladda om sidan med `Ctrl+F5`.
4. Logga in med `kontakt@offroadbumpis.se` och exakt samma värde som `ADMIN_PASSWORD`.

Vid varje omstart synkroniseras databasens administratörslösenord med `ADMIN_PASSWORD`.
